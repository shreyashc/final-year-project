...
const requireStartup = (_req: Request, res: Response, next: NextFunction) => {
  if (res.locals.user.role === "startup") {
    next();
  } else {
    next(new httpErrors.Unauthorized());
  }
...
export { requireAuth, requireAuthApi, requireAdmin, requireStartup };
